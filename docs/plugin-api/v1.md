...
| `modelScope` | `"channel"` | Optional. Accept models configured on channels bound to this type-61 plugin; omitted plugins remain restricted to `models`. |
...
`modelScope: "channel"` lets a type-61 plugin accept protocol models explicitly configured on a channel bound to that plugin, in addition to the defaults in `models`. It is intended for providers whose model catalog changes independently of the plugin. Distribution still requires the selected channel to list the requested model and bind the same `task_plugin_key`; it does not grant the plugin access to unrelated channels. Native plugin routes and per-protocol `models` restrictions remain static.

Plugins may optionally export `buildPerformanceRequest(ctx)` and `parsePerformanceResponse(ctx, body, response)` together. The host uses these hooks only as provider-observed health fallback metadata when a monitored local group has no local traffic samples. The parser returns model-level latency, success rate, throughput, and actual timestamped intervals; this data does not create local traffic, affect billing, or replace locally observed metrics. Exporting only one hook is invalid.
