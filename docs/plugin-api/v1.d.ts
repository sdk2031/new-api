...
export interface Meta {requiredCapabilities?: readonly HostCapability[]; submitResponseTypes?: readonly ("json" | "sse")[]; sortPriority?: number; website?: string; apiVersion: 1; key: string; name: string; icon?: string; description?: LocalizedText; version: string; author: {name: string; url?: string}; baseUrl?: string; channelTypes?: readonly number[]; models: readonly string[]; modelScope?: "channel"; fetchMode: "per_task" | "batch"; allowedHosts?: readonly string[]; routes?: readonly NativeRoute[]; protocols?: readonly ProtocolClaim[]; usageSchema?: Readonly<Record<string, UsageFieldSchema>>; usageExamples?: readonly UsageExample[]; usageProfiles?: readonly UsageProfile[]; auth?: "none" | "api_key" | "vertex_oauth" | {type: "none" | "api_key" | "oauth2_jwt"}}
...
export interface PerformanceRequestContext {baseUrl: string; hours: number; apiKey?: string; authHeader: string; auth?: unknown}
export interface PerformanceInterval {ts: number; avgLatencyMs: number; successRate: number; avgTps: number; requestCount: number}
export interface PerformanceModel {modelName: string; avgLatencyMs: number; successRate: number; avgTps: number; recentIntervals: readonly PerformanceInterval[]}
...
export declare function buildPerformanceRequest(ctx: PerformanceRequestContext): RequestDescriptor;
export declare function parsePerformanceResponse(ctx: PerformanceRequestContext, body: unknown, response: HookHTTPResponse): readonly PerformanceModel[];
